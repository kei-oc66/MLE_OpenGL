...
int main() {
  ...
  GLFWwindow *window = glfwCreateWindow(width, height, "NULLL", NULL, NULL);
  ...
  // Texture
  Texture brickTex((parentDir + texPath + "isagi.jpg").c_str(), GL_TEXTURE_2D,
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Tells OpenGL which Shader Program we want to use
    lightShader.Activate();
    // Export the camMatrix to the Vertex Shader of the light
    // cube
    camera.Matrix(lightShader, "camMatrix");
    // Bind the VAO so OpenGL knows to use it
    lightVAO.Bind();
    // Draw primitives, number of indices, datatype of indices,
    // index of indices
